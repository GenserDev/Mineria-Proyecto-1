...
##4 Entre los divorcios, predomina cierto nivel educativo (escohom/escomuj)
```{r}
if ("escohom" %in% names(df)) {
  t1 <- as.data.frame(table(df$escohom, useNA="ifany"))
  names(t1) <- c("escohom","n")
  t1$pct <- round(100*t1$n/sum(t1$n), 2)
  knitr::kable(t1, caption="Distribución educación esposo (escohom)")
}
if ("escomuj" %in% names(df)) {
  t2 <- as.data.frame(table(df$escomuj, useNA="ifany"))
  names(t2) <- c("escomuj","n")
  t2$pct <- round(100*t2$n/sum(t2$n), 2)
  knitr::kable(t2, caption="Distribución educación esposa (escomuj)")
}

```


##5 El registro ocurre cerca del mes de ocurrencia (mesreg ≈ mesocu).
```{r}
if (all(c("mesocu","mesreg") %in% names(df))) {
  iguales <- mean(df$mesocu == df$mesreg, na.rm=TRUE)
  cat("Proporción mesocu == mesreg:", round(iguales, 3), "\n\n")

  t_mes <- table(df$mesocu, df$mesreg, useNA="no")
  knitr::kable(as.data.frame.matrix(t_mes),
               caption="Mes ocurrencia (filas) vs mes registro (columnas)")
}
