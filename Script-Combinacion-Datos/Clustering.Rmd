...
cluster 1: Divorcios jóvenes.
cluster 2: Divorcios con edad madura
cluster 3: divorcios en edad madura.

<<<<<<< HEAD
=======
```{r silueta-final-corregida}
# 1. Gráfica de Silueta (Corregida)

fviz_silhouette(silkmsil, palette = "jco", print.summary = FALSE) +
  labs(title = "Calidad del Agrupamiento (Silueta) - Modelo Final")
```
>>>>>>> 75b80331f3977615de865caab62a718cc86811b6
```{r proporciones en los clusters}
table(kmsil$cluster)
```

```{r mapa-clusters-final}
fviz_cluster(kmsil, data = df_cluster_escalado,
             geom = "point",
             ellipse.type = "convex", 
             palette = "jco",
             ggtheme = theme_minimal(),
             main = "Mapa Final de Clusters")
```